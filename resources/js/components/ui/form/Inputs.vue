<template>
    <div :class="group_classes">
        <label v-show="!withoutLabel" :for="input_id">{{ label ?? ucword(field) }} <span v-show="!optional"
                :class="classes.label_class">*</span> <i v-show="fetchingData || loading"
                class="fa fa-spinner fa-spin"></i></label>
        <select v-if="type == 'select'" ref="input" :name="field" :disabled="disabled" :id="input_id"
            :class="{ 'is-invalid': form?.errors[field] }" :aria-describedby="input_id + '-error'" class="form-control"
            aria-invalid="true" @change="updateModelValue">
            <option value="">{{ label ?? ucword(field) }}</option>
            <option v-for="(option, index) in options" :value="option[trackBy]">{{ option[labelKey] }}</option>
        </select>
        <input v-else ref="input" :value="modelValue" :disabled="disabled" :required="!optional" :max="max" :min="min"
            :placeholder="placeholder" :name="field" :id="input_id" :class="{ 'is-invalid': form?.errors[field] }"
            :aria-describedby="input_id + '-error'" class="form-control" aria-invalid="true" :type="type"
            @input="updateModelValue" @keydown.down="heighlightNext" @keydown.up="heighlightPrevious"
            @keydown.enter="selectHeighlighted" />


        <div ref="list" v-show="autocompleteData && showSuggestion" class="autocomplete-container">
            <ul class="autocomplete-list">
                <li v-for="(item, index) in autocompleteData" :class="{ selected: index === heighlightIndex }"
                    @click="selectHeighlighted" @mouseenter="heighlightIndex = index" :key="index">
        ...
        <span :id="input_id + '-error'" class="error invalid-feedback" v-html="form?.errors[field]"></span>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        group_classes() {
            return this.classes.group_class + this.groupClass
        },
        input_id() {
